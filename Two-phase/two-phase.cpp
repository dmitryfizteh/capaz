...
void assign_P_Xi(ptr_Arrays HostArraysPtr, int i, int j, int k, localN locN, consts def)
{
	...
	double P_k = def.P_d[media] * pow((1. - HostArraysPtr.S_n[i+j*(locN.x)+k*(locN.x)*(locN.y)] - def.S_wr[media]) / (1. - def.S_wr[media]), -1. / def.lambda[media]);

	HostArraysPtr.P_n[i+j*(locN.x)+k*(locN.x)*(locN.y)] = HostArraysPtr.P_w[i+j*(locN.x)+k*(locN.x)*(locN.y)] + P_k;
...
void Newton(ptr_Arrays HostArraysPtr, int i, int j, int k, localN locN, consts def)
{
	if ((i!=0) && (i!=(locN.x)-1) && (j!=0) && (j!=(locN.y)-1) && (((k!=0) && (k!=(locN.z)-1)) || ((locN.z)<2)))
	{
		...
		for (int w=1;w<=def.newton_iterations;w++)
		{
			...
			F2 = def.ro0_n * (1. + (def.beta_n) * (HostArraysPtr.P_w[i+j*(locN.x)+k*(locN.x)*(locN.y)] + P_k - def.P_atm)) * HostArraysPtr.S_n[i+j*(locN.x)+k*(locN.x)*(locN.y)] - HostArraysPtr.roS_n[i+j*(locN.x)+k*(locN.x)*(locN.y)];
			...
			F2S = def.ro0_n * (1 + (def.beta_n) * (HostArraysPtr.P_w[i+j*(locN.x)+k*(locN.x)*(locN.y)] + P_k - def.P_atm + (HostArraysPtr.S_n[i+j*(locN.x)+k*(locN.x)*(locN.y)] * PkS)));
