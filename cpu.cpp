...
*/			// Если границы области "прозрачные"
			HostArraysPtr.uy_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
				((HostArraysPtr.P_g[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_g[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j+1) * (def.hy)) 
				- (HostArraysPtr.P_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * j * (def.hy)))/ def.hy;
#endif
			HostArraysPtr.uy_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
				((HostArraysPtr.P_w[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_w[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j+1) * (def.hy)) 
				- (HostArraysPtr.P_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * j * (def.hy)))/ def.hy; 
			HostArraysPtr.uy_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
				((HostArraysPtr.P_n[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_n[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j+1) * (def.hy)) 
				- (HostArraysPtr.P_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * j * (def.hy)))/ def.hy; 
		}
		else
		{
			if (j == (def.locNy) - 1)
...
#ifdef THREE_PHASE
				HostArraysPtr.uy_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
					((HostArraysPtr.P_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * j * (def.hy)) 
					- (HostArraysPtr.P_g[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_g[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j-1) * (def.hy)))/ def.hy;
#endif
				HostArraysPtr.uy_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
				((HostArraysPtr.P_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * j * (def.hy)) 
					- (HostArraysPtr.P_w[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_w[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j-1) * (def.hy)))/ def.hy;
				HostArraysPtr.uy_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
					((HostArraysPtr.P_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * j * (def.hy)) 
					- (HostArraysPtr.P_n[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_n[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j-1) * (def.hy)))/ def.hy;
			}
			else
			{
				HostArraysPtr.uy_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
					((HostArraysPtr.P_w[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_w[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j+1) * (def.hy)) 
					- (HostArraysPtr.P_w[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_w[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j-1) * (def.hy)))/ (2*def.hy);
				HostArraysPtr.uy_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
					((HostArraysPtr.P_n[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_n[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j+1) * (def.hy)) 
					- (HostArraysPtr.P_n[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_n[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j-1) * (def.hy)))/ (2*def.hy);
#ifdef THREE_PHASE
				HostArraysPtr.uy_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] = HostArraysPtr.Xi_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * 
					((HostArraysPtr.P_g[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_g[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j+1) * (def.hy)) 
					- (HostArraysPtr.P_g[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - HostArraysPtr.ro_g[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const) * (j-1) * (def.hy)))/ (2*def.hy);
#endif
			}
		}
	}
	...
	test_nan(HostArraysPtr.ux_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
	test_nan(HostArraysPtr.ux_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
	test_nan(HostArraysPtr.uy_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
	test_nan(HostArraysPtr.uy_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
	test_nan(HostArraysPtr.uz_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
	test_nan(HostArraysPtr.uz_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
#ifdef THREE_PHASE
	test_nan(HostArraysPtr.ux_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
	test_nan(HostArraysPtr.uy_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
	test_nan(HostArraysPtr.uz_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], __FILE__, __LINE__);
#endif
}
...
void assign_roS(ptr_Arrays HostArraysPtr, double t, int i, int j, int k, consts def)
{
	if ((i!=0) && (i!=(def.locNx)-1) && (j!=0) && (j!=(def.locNy)-1) && (((k!=0) && (k!=(def.locNz)-1)) || ((def.Nz)<2)))
	{
		...
		else
		{
			...
			divgrad3 = (def.m[media] * (def.l) * (def.c_n) / 2.) 
		...
		{
			A1 = HostArraysPtr.roS_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] + ((def.dt) / def.m[media]) * (q_w + divgrad1 - Tx1 - Ty1 - Tz1);
			A2 = HostArraysPtr.roS_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] + ((def.dt) / def.m[media]) * (q_n + divgrad2 - Tx2 - Ty2 - Tz2);
#ifdef THREE_PHASE
			A3 = HostArraysPtr.roS_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] + ((def.dt) / def.m[media]) * (divgrad3 - Tx3 - Ty3 - Tz3);
#endif
		}
		else
		{
			A1 = (1. / ((def.m[media]) * (def.dt) + 2 * (def.tau))) * (2 * (def.dt) * (def.dt) * (q_w + divgrad1 - Tx1 - Ty1 - Tz1) 
				+ ((def.m[media]) * (def.dt) - 2 * (def.tau)) * HostArraysPtr.roS_w_old[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] 
			...
			A2 = (1. / ((def.m[media]) * (def.dt) + 2 * (def.tau))) * (2 * (def.dt) * (def.dt) * (q_n + divgrad1 - Tx1 - Ty1 - Tz1) 
				+ ((def.m[media]) * (def.dt) - 2 * (def.tau)) * HostArraysPtr.roS_n_old[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] 
				+ 4 * (def.tau) * HostArraysPtr.roS_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)]);  

			test_tau(HostArraysPtr.roS_w_old[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], HostArraysPtr.roS_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], A1, HostArraysPtr.media[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], def, __FILE__, __LINE__);
			test_tau(HostArraysPtr.roS_n_old[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], HostArraysPtr.roS_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], A2, HostArraysPtr.media[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)], def, __FILE__, __LINE__);

#ifdef THREE_PHASE
			A3 = (1. / ((def.m[media]) * (def.dt) + 2 * (def.tau))) * (2 * (def.dt) * (def.dt) * (divgrad1 - Tx1 - Ty1 - Tz1) 
				+ ((def.m[media]) * (def.dt) - 2 * (def.tau)) * HostArraysPtr.roS_g_old[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] 
...
void assign_roS_nr(ptr_Arrays HostArraysPtr, double t, int i, int j, int k, consts def)
{
	if ((i!=0) && (i!=(def.locNx)-1) && (j!=0) && (j!=(def.locNy)-1) && (((k!=0) && (k!=(def.locNz)-1)) || ((def.locNz)<2)))
	{
		...
		else
		{
			...
			f3 = (((z2 + abs(z2))/2.0 - (z1 - abs(z1))/2.0)*(-1) * HostArraysPtr.Xi_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
                      (z1 + abs(z1))/2.0*(-1)* HostArraysPtr.Xi_w[i+j*(def.locNx)+(k-1)*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i+j*(def.locNx)+(k-1)*(def.locNx)*(def.locNy)] +
                      (z2 - abs(z2))/2.0*(-1)* HostArraysPtr.Xi_w[i+j*(def.locNx)+(k+1)*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i+j*(def.locNx)+(k+1)*(def.locNx)*(def.locNy)])/def.hz;
			...
			g3 = (((z2 + abs(z2))/2.0 - (z1 - abs(z1))/2.0)*(-1) * HostArraysPtr.Xi_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
                      (z1 + abs(z1))/2.0*(-1)* HostArraysPtr.Xi_n[i+j*(def.locNx)+(k-1)*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i+j*(def.locNx)+(k-1)*(def.locNx)*(def.locNy)] +
                      (z2 - abs(z2))/2.0*(-1)* HostArraysPtr.Xi_n[i+j*(def.locNx)+(k+1)*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i+j*(def.locNx)+(k+1)*(def.locNx)*(def.locNy)])/def.hz;
#ifdef THREE_PHASE
			...
			b3 = (((z2 + abs(z2))/2.0 - (z1 - abs(z1))/2.0)*(-1) * HostArraysPtr.Xi_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
				(z1 + abs(z1))/2.0*(-1)* HostArraysPtr.Xi_g[i+j*(def.locNx)+(k-1)*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i+j*(def.locNx)+(k-1)*(def.locNx)*(def.locNy)] +
				(z2 - abs(z2))/2.0*(-1)* HostArraysPtr.Xi_g[i+j*(def.locNx)+(k+1)*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i+j*(def.locNx)+(k+1)*(def.locNx)*(def.locNy)])/def.hz;
#endif
		}
        ...
        y2 = -(HostArraysPtr.P_w[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - Pw)/def.hy + HostArraysPtr.ro_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const);
        y1 = -(Pw - HostArraysPtr.P_w[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hy + HostArraysPtr.ro_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const);

        f1 = (((x2 + abs(x2))/2.0 - (x1 - abs(x1))/2.0)*(-1) * HostArraysPtr.Xi_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
                (x1 + abs(x1))/2.0*(-1)* HostArraysPtr.Xi_w[i-1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i-1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] +
                (x2 - abs(x2))/2.0*(-1)* HostArraysPtr.Xi_w[i+1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i+1+j*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hx;

        f2 = (((y2 + abs(y2))/2.0 - (y1 - abs(y1))/2.0)*(-1)* HostArraysPtr.Xi_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
                (y1 + abs(y1))/2.0*(-1)* HostArraysPtr.Xi_w[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] +
                (y2 - abs(y2))/2.0*(-1)* HostArraysPtr.Xi_w[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_w[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hy;
        ...
        y2 = -(HostArraysPtr.P_n[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - Pn)/def.hy + HostArraysPtr.ro_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const);
        y1 = -(Pn - HostArraysPtr.P_n[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hy + HostArraysPtr.ro_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const);

        g1 = (((x2 + abs(x2))/2.0 - (x1 - abs(x1))/2.0)*(-1) * HostArraysPtr.Xi_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
                (x1 + abs(x1))/2.0*(-1)* HostArraysPtr.Xi_n[i-1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i-1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] +
                (x2 - abs(x2))/2.0*(-1)* HostArraysPtr.Xi_n[i+1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i+1+j*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hx;

        g2 = (((y2 + abs(y2))/2.0 - (y1 - abs(y1))/2.0)*(-1)* HostArraysPtr.Xi_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
                (y1 + abs(y1))/2.0*(-1)* HostArraysPtr.Xi_n[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] +
                (y2 - abs(y2))/2.0*(-1)* HostArraysPtr.Xi_n[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_n[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hy;

		A1 = HostArraysPtr.roS_w[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - (def.dt/def.m[media])*(q_w + f1 + f2 + f3);
        A2 = HostArraysPtr.roS_n[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - (def.dt/def.m[media])*(q_n + g1 + g2 + g3);
		...
		y2 = -(HostArraysPtr.P_g[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] - Pg)/def.hy + HostArraysPtr.ro_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const);
		y1 = -(Pg - HostArraysPtr.P_g[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hy + HostArraysPtr.ro_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * (def.g_const);

		b1 = (((x2 + abs(x2))/2.0 - (x1 - abs(x1))/2.0)*(-1) * HostArraysPtr.Xi_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
			(x1 + abs(x1))/2.0*(-1)* HostArraysPtr.Xi_g[i-1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i-1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] +
			(x2 - abs(x2))/2.0*(-1)* HostArraysPtr.Xi_g[i+1+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i+1+j*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hx;

		b2 = (((y2 + abs(y2))/2.0 - (y1 - abs(y1))/2.0)*(-1)* HostArraysPtr.Xi_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] -
			(y1 + abs(y1))/2.0*(-1)* HostArraysPtr.Xi_g[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i+(j-1)*(def.locNx)+k*(def.locNx)*(def.locNy)] +
			(y2 - abs(y2))/2.0*(-1)* HostArraysPtr.Xi_g[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)] * HostArraysPtr.ro_g[i+(j+1)*(def.locNx)+k*(def.locNx)*(def.locNy)])/def.hy;

		A3 = HostArraysPtr.roS_g[i+j*(def.locNx)+k*(def.locNx)*(def.locNy)] - (def.dt/def.m[media])*(b1 + b2 + b3);
