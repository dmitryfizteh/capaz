...
// Номер шага по времени, на котором нужно сделать отдельное сохранение:
//0
#define Vrema_break 466562
...
//bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb	
	// в случае решения эллиптич. ур-я в 3-фазном случае это давление газа.
	double value;
...
//аналоги ф-ии divgrad() для 3-фазной задачи:
double step_0_divgrad1(int i,int j);
double step_0_divgrad2(int i,int j);
...
//eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
	...
	//Давление газа сначала заполняется константой
	p[i][j].value=P_g;
...
void rec()
{
	...
	sprintf(namefl, "output\\p_w\\pw_%d.dat",vrema+10000000);
    fp_w=fopen(namefl,"w");
	sprintf(namefl, "output\\p_n\\pn_%d.dat",vrema+10000000);
    fp_n=fopen(namefl,"w");
	sprintf(namefl, "output\\p_g\\pg_%d.dat",vrema+10000000);
    fp_g=fopen(namefl,"w");
...
//eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
	...
	fprintf(fp_g,"TITLE =  \"Pressure\" \n");
	fprintf(fp_g,"VARIABLES = \"X\",\"Y\",\"Pg\" \n");
    fprintf(fp_g,"ZONE T = \"BIG ZONE\", J=%d  ,I=%d, F = POINT \n", N2, N1);
	...
	for(j=0; j<N2; j++)
	{
			for(i=0; i<N1; i++)
			{
				 ...
				 //временный тест:
				 //fprintf(fp_n,"%6.4e %6.4e %6.4e \n",x[i],y[j],pnt[i][j].f_value[0].p_cgw);
				 fprintf(fp_g,"%6.4e %6.4e %6.4e \n",x[i],y[j],p[i][j].value);
			}
	 }
...
void step_time()
{
	...
	if(vrema==0)
	{
		...
		step_2();
		// вычисление остальных давлений по этому давлению газа
		step_0();
	}
	step_1();
	//test_step_1();
	...
	if(vrema%M_out==0 || vrema==Vrema_break)
	{
	...
	for(i=0;i<N1;i++)
	{
		for(j=0;j<N2;j++)
		{
			for(v=0;v<2;v++)
			{
				...
				//значение a_zeid:
				pnt[i][j].f_value[v].a_zeid=k[sn]*(pnt[i][j].f_value[v].k_rw/mu_w+pnt[i][j].f_value[v].k_rn/mu_n+pnt[i][j].f_value[v].k_rg/mu_g);
...
void step_0_b()
{
	...
	double l_f1, l_f2, l_f3, l_f4, l_f5;
	...
	for(i=1;i<N1-1;i++)
	{
		for(j=1;j<N2-1;j++)
		{
			...
			l_f1 = ro_w*g/mu_w*(pnt[i][j].f_value[v.u].k_rw* k[sn]-pnt[i][j-1].f_value[v.u].k_rw*k[sn])/h2;
			l_f2 = ro_n*g/mu_n*(pnt[i][j].f_value[v.u].k_rn* k[sn]-pnt[i][j-1].f_value[v.u].k_rn*k[sn])/h2;
			l_f3 = ro_g*g/mu_g*(pnt[i][j].f_value[v.u].k_rg* k[sn]-pnt[i][j-1].f_value[v.u].k_rg*k[sn])/h2;
			l_f4 = step_0_divgrad1(i,j)/mu_n;
			l_f5 = step_0_divgrad2(i,j)/mu_w;
			...
			pnt[i][j].rel.f=-(l_f1+l_f2+l_f3+l_f4+l_f5);
			//Учёт источника:
...
double step_0_divgrad1(int i,int j)
{
	//вычисляется div(k*k_rn* grad(P_cgn))
	...
	b_x2=(pnt[i+1][j].f_value[v.r].p_cgn-pnt[i][j].f_value[v.r].p_cgn)/h1;
	b_x1=(pnt[i][j].f_value[v.l].p_cgn-pnt[i-1][j].f_value[v.l].p_cgn)/h1;
	b_y2=(pnt[i][j+1].f_value[v.d].p_cgn-pnt[i][j].f_value[v.d].p_cgn)/h2;
	b_y1=(pnt[i][j].f_value[v.u].p_cgn-pnt[i][j-1].f_value[v.u].p_cgn)/h2;
	ax2=(pnt[i+1][j].f_value[v.r].k_rn+pnt[i][j].f_value[v.r].k_rn)*k[pnt[i][j].f_value[v.r].sand]/2;
	ax1=(pnt[i][j].f_value[v.l].k_rn+pnt[i-1][j].f_value[v.l].k_rn)*k[pnt[i][j].f_value[v.l].sand]/2;
	ay2=(pnt[i][j+1].f_value[v.d].k_rn+pnt[i][j].f_value[v.d].k_rn)*k[pnt[i][j].f_value[v.d].sand]/2;
	ay1=(pnt[i][j].f_value[v.u].k_rn+pnt[i][j-1].f_value[v.u].k_rn)*k[pnt[i][j].f_value[v.u].sand]/2;
	return (ax2*b_x2-ax1*b_x1)/h1+(ay2*b_y2-ay1*b_y1)/h2;
...
double step_0_divgrad2(int i,int j)
{
	//вычисляется div(k*k_rw* grad(P_cgw))
	...
	b_x2=(pnt[i+1][j].f_value[v.r].p_cgw-pnt[i][j].f_value[v.r].p_cgw)/h1;
	b_x1=(pnt[i][j].f_value[v.l].p_cgw-pnt[i-1][j].f_value[v.l].p_cgw)/h1;
	b_y2=(pnt[i][j+1].f_value[v.d].p_cgw-pnt[i][j].f_value[v.d].p_cgw)/h2;
	b_y1=(pnt[i][j].f_value[v.u].p_cgw-pnt[i][j-1].f_value[v.u].p_cgw)/h2;
	ax2=(pnt[i+1][j].f_value[v.r].k_rw+pnt[i][j].f_value[v.r].k_rw)*k[pnt[i][j].f_value[v.r].sand]/2;
	ax1=(pnt[i][j].f_value[v.l].k_rw+pnt[i-1][j].f_value[v.l].k_rw)*k[pnt[i][j].f_value[v.l].sand]/2;
	ay2=(pnt[i][j+1].f_value[v.d].k_rw+pnt[i][j].f_value[v.d].k_rw)*k[pnt[i][j].f_value[v.d].sand]/2;
	ay1=(pnt[i][j].f_value[v.u].k_rw+pnt[i][j-1].f_value[v.u].k_rw)*k[pnt[i][j].f_value[v.u].sand]/2;
	return (ax2*b_x2-ax1*b_x1)/h1+(ay2*b_y2-ay1*b_y1)/h2;
...
void step_0_pressure(int i,int j, int v)
{
	...
	//расчет p_n и p_w:
	p[i][j].n[v]=p[i][j].value-pnt[i][j].f_value[v].p_cgn;
	//p[i][j].w[v]=P_g-(pnt[i][j].f_value[v].p_cgn+pnt[i][j].f_value[v].p_cnw);
	pnt[i][j].f_value[v].p_cgw = pnt[i][j].f_value[v].p_cgn+pnt[i][j].f_value[v].p_cnw;
	p[i][j].w[v]=p[i][j].value-pnt[i][j].f_value[v].p_cgw;
}
...
void step_2_gr_after()
{
	...
	for(j=0;j<N2;j++)
	{
		p_next[0][j].value=p_next[1][j].value;
		p_next[N1-1][j].value=p_next[N1-2][j].value;
	}
...
void save()
{
	...
	if(vrema==Vrema_break) sprintf(namefl, "save\\S_%d.set",vrema+10000000);
...
void load()
{
	...
	if((fp=fopen("save\\S_10466562.set","rb"))==NULL)
	//if((fp=fopen("save\\S1093.set","rb"))==NULL)
		printf("Не открыл\n\n\n\n");
